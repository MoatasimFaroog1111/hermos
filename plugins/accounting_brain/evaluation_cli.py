...
def register_evaluation_cli(parser: argparse.ArgumentParser) -> None:
    ...
    grounded = subs.add_parser(
        "run-grounded",
        help=(
            "Run the holdout with leakage-safe GITC company memory plus earlier "
            "Gold historical retrieval"
        ),
    )
    grounded.add_argument("--top-k", type=int, default=5)
    grounded.add_argument("--timeout-seconds", type=float, default=120.0)
    grounded.add_argument("--max-tokens", type=int, default=1800)
...
def evaluation_command(args: argparse.Namespace) -> int:
    ...
    try:
        if action == "run":
            ...
            print(
                "Choose one action: run, prepare-reference, run-retrieval, "
                "run-grounded, or score"
            )
            return 2
    except (
        BaselineEvaluationError,
...
def _top_k(args: argparse.Namespace) -> int:
    return max(1, min(10, int(args.top_k)))
