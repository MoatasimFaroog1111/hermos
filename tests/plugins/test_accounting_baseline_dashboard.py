...
def test_dashboard_baseline_uses_existing_runner_and_stays_safe(
            ...
            progress_callback(
                {
                    ...
                    "repairs_attempted": 0,
                    "elapsed_seconds": 1.0,
                    "last_case_duration_seconds": 1.0,
                    "last_model_call_duration_seconds": 0.9,
                }
            )
        return {
            "ok": False,
    ...
    assert captured["progress_callback"] is plugin_api_v2._record_progress
    assert plugin_api_v2._BASELINE_STATE["progress"]["phase"] == "case_completed"
    assert plugin_api_v2._BASELINE_STATE["progress"]["completed_cases"] == 1
